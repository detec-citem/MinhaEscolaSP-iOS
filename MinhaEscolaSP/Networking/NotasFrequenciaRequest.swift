...
final class NotasFrequenciaRequest {
    ...
    //MARK: Methods
    static func buscarFrequencias(aluno: Aluno, turma: Turma, completion: @escaping ([Frequencia]?,Bool,String?) -> Void) {
        if !Requests.Configuracoes.servidorHabilitado || LoginRequest.usuarioLogado.usuarioMock(), let json = Requests.getLocalJsonData(name: Constants.notasFrequenciasMock) as? [[String:Any]] {
            salvarFrequencias(aluno: aluno, frequenciasJson: json, completion: completion)
        }
    ...
    fileprivate static func salvarFrequencias(aluno: Aluno, frequenciasJson: [[String:Any]], completion: @escaping ([Frequencia]?,Bool,String?) -> Void) {
        DispatchQueue.main.async {
            var frequencias = [Frequencia]()
            for frequenciaJson in frequenciasJson {
                if let bimestre = frequenciaJson[Constants.bimestre] as? UInt16, let frequenciasJson = frequenciaJson[Constants.notas] as? [[String:Any]] {
                    for frequenciaJson in frequenciasJson {
                        let frequencia = FrequenciaDao.salvarFrequencia(bimestre: bimestre, json: frequenciaJson, aluno: aluno)
                        frequencias.append(frequencia)
                    }
                }
            }
            CoreDataManager.sharedInstance.saveContext()
            completion(frequencias, true, nil)
        }
    }
